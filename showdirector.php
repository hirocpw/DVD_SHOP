...
<body>
		...
		<?php
			...
			{
			$genre_name = mysqli_real_escape_string($db,$_POST['director_name']);
			$query = "SELECT *
					  FROM movie_detail_view
					  WHERE director LIKE '%$genre_name%'";
			...
			print("<form class = \"search\" method = \"post\" action = \"showdirector.php \">");
			

			$director_query = "SELECT * 
							FROM director ORDER BY director_name ASC";
			$result0 = mysqli_query($db,$director_query);
